...
class TestReservation(MyTest):
    ...
    def test12_post_reservation(self):
        ...
        a = ast.literal_eval(resp.data.decode('utf-8'))
        idres = a['idReservation']
        f = open("idreservation.txt", "w")
        f.write(str(idres))
        f.close()
        self.assertEqual(200, resp.status_code)

    def test13_get_reservation(self):
        f = open("idreservation.txt", 'r')
        resp = self.client.get("http://localhost:5000/reservation/" + f.read(),
                               headers=self.header,
                               auth=self.user_auth)
    ...
    def test16_put_reservation(self):
        f = open("idreservation.txt", 'r')
        resp = self.client.put("http://localhost:5000/reservation/" + f.read(),
                               headers=self.header,
                               auth=self.user_auth,
    ...
    def test18_delete_reservation(self):
        ...
        resp = self.client.delete(
            "http://localhost:5000/reservation/" + f.read(),
            headers=self.header,
            auth=self.user_auth)
        self.assertEqual(resp.status_code, 200)
